...
class ThanhLy(models.Model):
    ...
    ma_thanh_ly = fields.Char("Ma thanh ly", required=True, copy=False, readonly=True, default="New")
    ...
    gia_tri_con_lai = fields.Float("Gia tri con lai (VND)", related="tai_san_id.gia_tri_con_lai", readonly=True)
    gia_tri_thanh_ly = fields.Float("Gia tri thu hoi (VND)")
    lai_lo_thanh_ly = fields.Float("Lai/(Lo) thanh ly (VND)", compute="_compute_lai_lo", store=True)

    trang_thai = fields.Selection([
        ("cho_duyet", "Cho duyet"),
        ("da_duyet", "Da duyet"),
        ("huy", "Da huy"),
    ], string="Trang thai", default="cho_duyet", tracking=True)
    ...
    def create(self, vals):
        if vals.get("ma_thanh_ly", "New") == "New":
            ...
            num = int(last.ma_thanh_ly[2:]) if last and last.ma_thanh_ly.startswith("TL") else 0
    ...
    def action_duyet(self):
        for rec in self:
            ...
            rec.message_post(body="Phieu thanh ly da duoc duyet. Tai san chuyen sang trang thai Da thanh ly.")

            # TRIGGER Muc 2: tu dong tao phieu thu tai chinh
            if rec.gia_tri_thanh_ly > 0:
                nguoi_lap = self.env["nhan_vien"].search([], limit=1)
                self.env["tai_chinh.phieu_thu_chi"].create({
                    "loai_phieu": "thu",
                    "nguon_goc": "thanh_ly",
                    "ten_noi_dung": "Thu tu thanh ly tai san: %s (%s)" % (rec.tai_san_id.ten_tai_san, rec.ma_thanh_ly),
                ...
                rec.message_post(body="Tu dong tao phieu thu tai chinh: %s VND" % rec.gia_tri_thanh_ly)
