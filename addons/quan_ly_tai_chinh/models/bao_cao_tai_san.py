...
class BaoCaoTaiSan(models.Model):
    ...
    _description = "Bao cao tai san tong hop"
    _rec_name = "ma_bao_cao"
    _order = "nam_bao_cao desc, id desc"
    _inherit = ["mail.thread", "mail.activity.mixin"]

    ma_bao_cao = fields.Char(
        "Ma bao cao", required=True, copy=False, readonly=True, default="New"
    )
    nam_bao_cao = fields.Integer(
        "Nam bao cao", required=True,
        default=lambda self: __import__("datetime").date.today().year
    )
    ngay_lap = fields.Date("Ngay lap", default=fields.Date.today, required=True)
    nguoi_lap_id = fields.Many2one("nhan_vien", string="Nguoi lap")
    ...
    tong_nguyen_gia = fields.Float("Tong nguyen gia (VND)", readonly=True)
    tong_da_khau_hao = fields.Float("Tong da khau hao (VND)", readonly=True)
    tong_gia_tri_con_lai = fields.Float("Tong gia tri con lai (VND)", readonly=True)
    ...
    chi_tiet_ids = fields.One2many(
        "tai_chinh.bao_cao_tai_san.chi_tiet", "bao_cao_id", string="Chi tiet theo loai"
    )

    # === AI ANALYSIS (Gemini) ===
    phan_tich_ai = fields.Text("Phan tich AI (Gemini)", readonly=True)
    trang_thai_ai = fields.Selection([
        ("chua_phan_tich", "Chua phan tich"),
        ("hoan_thanh", "Hoan thanh"),
        ("loi", "Loi"),
    ], string="Trang thai AI", default="chua_phan_tich", readonly=True)
    ...
    def create(self, vals):
        if vals.get("ma_bao_cao", "New") == "New":
            ...
            num = 0
            if last and last.ma_bao_cao and last.ma_bao_cao.startswith("BCTS"):
                try:
                    num = int(last.ma_bao_cao[4:])
                except ValueError:
                    pass
    ...
    def action_tinh_toan(self):
        for rec in self:
            ...
            for loai in loai_ts:
                ts_loai = tai_san_all.filtered(
                    lambda t: t.danh_muc_loai_tai_san_id.id == loai.id
                )
                if not ts_loai:
                    continue
            ...
            rec.phan_tich_ai = False
            rec.trang_thai_ai = "chua_phan_tich"
    ...
    def action_phan_tich_ai(self):
        """Goi Gemini API de phan tich bao cao tai san."""
        from ..utils.gemini_helper import phan_tich_tai_san
        for rec in self:
            if not rec.tong_tai_san:
                raise UserError("Vui long bam 'Tinh toan' truoc khi phan tich AI.")
            try:
                ket_qua = phan_tich_tai_san(self.env, rec)
                rec.phan_tich_ai = ket_qua
                rec.trang_thai_ai = "hoan_thanh"
                rec.message_post(body="AI (Gemini) da phan tich bao cao thanh cong.")
            except Exception as e:
                rec.phan_tich_ai = "Loi khi goi Gemini API: %s" % str(e)
                rec.trang_thai_ai = "loi"
...
class BaoCaoTaiSanChiTiet(models.Model):
    ...
    bao_cao_id = fields.Many2one(
        "tai_chinh.bao_cao_tai_san", string="Bao cao", required=True, ondelete="cascade"
    )
    danh_muc_loai_tai_san_id = fields.Many2one("danh_muc_loai_tai_san", string="Loai tai san")
    so_tai_san = fields.Integer("So tai san")
    tong_nguyen_gia = fields.Float("Tong nguyen gia (VND)")
    tong_da_khau_hao = fields.Float("Tong da khau hao (VND)")
    tong_gia_tri_con_lai = fields.Float("Tong gia tri con lai (VND)")
    ty_le_con_lai = fields.Float("% Con lai", compute="_compute_ty_le", store=True)
