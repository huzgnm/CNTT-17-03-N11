...
class KhauHaoHangThang(models.Model):
    ...
    ma_khau_hao = fields.Char(
        "Ma khau hao", required=True, copy=False, readonly=True, default="New"
    )
    tai_san_id = fields.Many2one("tai_san", string="Tai san", required=True, ondelete="cascade")
    ...
    danh_muc_loai_tai_san_id = fields.Many2one(
        related="tai_san_id.danh_muc_loai_tai_san_id", string="Loai tai san", readonly=True
    )

    ky_khau_hao = fields.Date("Ky khau hao (thang)", required=True)
    gia_tri_truoc_kh = fields.Float("Gia tri truoc KH (VND)", readonly=True)
    so_tien_khau_hao = fields.Float("So tien khau hao (VND)", required=True)
